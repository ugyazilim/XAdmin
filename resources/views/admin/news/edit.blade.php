...
@section('title', 'Haber/Duyuru Düzenle')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="mb-1">Haber/Duyuru Düzenle</h2>
        <p class="text-muted mb-0">{{ $news->title }}</p>
    </div>
                ...
                <div class="col-md-8">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                            <option value="news" {{ old('type', $news->type) === 'news' ? 'selected' : '' }}>Haber</option>
                            <option value="announcement" {{ old('type', $news->type) === 'announcement' ? 'selected' : '' }}>Duyuru</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" class="form-control @error('published_at') is-invalid @enderror" 
                                   id="published_at" name="published_at" value="{{ old('published_at', $news->published_at ? $news->published_at->format('Y-m-d') : date('Y-m-d')) }}">
                            @error('published_at')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="card bg-light">
                        <div class="card-body">
                            ...
                            @if($news->featured_image)
                                <div class="mb-3">
                                    <img src="{{ asset($news->featured_image) }}" alt="{{ $news->title }}" class="img-fluid rounded mb-2">
                                </div>
                            @endif
                            <div class="mb-3">
